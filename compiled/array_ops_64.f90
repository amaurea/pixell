...
subroutine matmul_multi_sym(A, b, npix, n, k) bind(c)
	...
	real(c_double),        intent(in)    :: A(n,n,npix)
	real(c_double),        intent(inout) :: b(n,k,npix)
	real(c_double)    :: x(size(b,1),size(b,2))
	integer(4) :: i
...
subroutine matmul_multi(A, b, x, npix, n, m, k) bind(c)
	...
	real(c_double),        intent(in)    :: A(m,n,npix)
	real(c_double),        intent(in)    :: b(m,k,npix)
	real(c_double),        intent(inout) :: x(n,k,npix)
	integer(4) :: i
...
subroutine ang2rect(ang, rect, n) bind(c)
	...
	real(c_double), intent(in)        :: ang(2,n)
	real(c_double), intent(inout)     :: rect(3,n)
	real(c_double) :: st,ct,sp,cp
	integer :: i
...
subroutine find_contours(imap, vals, omap, ny, nx, nv) bind(c)
	...
	real(c_double),        intent(in)    :: imap(nx,ny), vals(nv)
...
subroutine roll_rows(imap, offsets, omap, ny, nx) bind(c)
	...
	real(c_double),        intent(in)    :: imap(nx,ny)
	integer(c_int), intent(in)    :: offsets(ny)
	real(c_double),        intent(inout) :: omap(nx,ny)
	integer :: y, ix, ox
