...
pure function sinc(x)
	...
	real(c_double) :: sinc, y
	real(c_double), parameter :: cut = 1e-4, pi = 3.14159265358979323846d0
	y = abs(pi*x)
	if(y < cut) then
...
subroutine spline_filter1d(data, dims, axis, ndim, ndata, order, border, trans) bind(c)
	implicit none
	real(c_double), intent(inout)     :: data(ndata)
	integer(c_int), intent(in) :: dims(ndim)
	integer(c_int), value      :: axis, order, border, trans, ndim, ndata
	real(c_double), allocatable   :: a(:)
	real(c_double), parameter     :: tolerance = 1d-15
	real(c_double) :: pole(2), p, weight, pn, p2n, ip, q, v
...
subroutine interpol(idata, ishape, odata, pos, npre, ndim, ngrid, nsamp, type, order, border, trans) bind(c)
	implicit none
	real(c_double), intent(inout) :: idata(ngrid,npre), odata(nsamp,npre)
	real(c_double), intent(in)    :: pos(nsamp,ndim)
	...
	real(c_double) :: v(size(idata,2)), res(size(idata,2))
...
subroutine interpol_deriv(idata, ishape, odata, pos, npre, ndim, ngrid, nsamp, type, order, border, trans) bind(c)
	implicit none
	real(c_double), intent(inout) :: idata(ngrid,npre), odata(nsamp,npre,ndim) ! (ngrid,npre) and (npoint,npre,ndim)
	real(c_double), intent(in)    :: pos(nsamp,ndim)                 ! (npoint,ndim)
	integer(c_int), intent(in) :: ishape(ndim)
	integer(c_int), value  :: npre, ndim, ngrid, nsamp, type, order, border, trans
	real(c_double), allocatable   :: weights(:,:), dweights(:,:)
	real(c_double) :: v(size(idata,2)), res(size(idata,2),size(pos,2)) ! (npre), (npre,ndim)
	integer :: off(size(pos,2)), inds(size(pos,2))
	integer :: xi, si, ci, i, j, dind, nw, ncon, n
