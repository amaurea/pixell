...
subroutine matmul_multi_sym(A, b, npix, n, k) bind(c)
	...
	real(_),        intent(in)    :: A(n,n,npix)
	real(_),        intent(inout) :: b(n,k,npix)
...
subroutine matmul_multi(A, b, x, npix, n, m, k) bind(c)
	...
	real(_),        intent(in)    :: A(m,n,npix)
	real(_),        intent(in)    :: b(m,k,npix)
	real(_),        intent(inout) :: x(n,k,npix)
	integer(4) :: i
...
subroutine ang2rect(ang, rect, n) bind(c)
	...
	real(_), intent(in)        :: ang(2,n)
	real(_), intent(inout)     :: rect(3,n)
	real(_) :: st,ct,sp,cp
	integer :: i
...
subroutine find_contours(imap, vals, omap, ny, nx, nv) bind(c)
	...
	real(_),        intent(in)    :: imap(nx,ny), vals(nv)
...
subroutine roll_rows(imap, offsets, omap, ny, nx) bind(c)
	...
	real(_),        intent(in)    :: imap(nx,ny)
	integer(c_int), intent(in)    :: offsets(ny)
	real(_),        intent(inout) :: omap(nx,ny)
	integer :: y, ix, ox
